...
def clean_boto3_import():
    ...
    # Save original state
    boto3_modules = {mod: sys.modules[mod] for mod in list(sys.modules.keys())
...
def test_s3_lazy_loading(clean_boto3_import):
    ...
    assert 'boto3' not in sys.modules, "boto3 should not be imported on peachbase import"
...
def test_s3_lazy_loading_on_use():
    ...
    with mock_aws():
        ...
        try:
            ...
            # Operation may fail, but boto3 should be loaded
            pass

        assert s3._s3_client._boto3 is not None, "boto3 should be loaded after S3 operation"


@pytest.mark.skipif(
    not pytest.importorskip('boto3', reason='boto3 not installed') or
    not pytest.importorskip('moto', reason='moto not installed'),
...
class TestS3Operations:
    ...
    def test_list_s3_collections_with_files(self):
        ...
        with mock_aws():
            ...
            # Upload test files
            s3.put_object(Bucket='test-bucket', Key='collection1.pdb', Body=b'test data 1')
            s3.put_object(Bucket='test-bucket', Key='collection2.pdb', Body=b'test data 2')
            s3.put_object(Bucket='test-bucket', Key='not_a_collection.txt', Body=b'other')
    ...
    def test_list_s3_collections_with_prefix(self):
        ...
        with mock_aws():
            ...
            s3.put_object(Bucket='test-bucket', Key='col4.pdb', Body=b'data4')  # root level
    ...
    def test_delete_s3_object(self):
        ...
        from moto import mock_aws
        from peachbase.utils.s3 import delete_s3_object, check_s3_object_exists
        import boto3
